...
# Force tqdm to use single-line mode
...
def train_epoch(model, dataloader, criterion, optimizer, device, epoch):
    """Train for one epoch with clean single-line progress bar"""
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
    ...
    # Create progress bar for batches - simplified for terminal compatibility
    progress_bar = tqdm(
    ...
    for batch_idx, (data, target) in progress_bar:
        ...
        # Update progress bar less frequently to prevent line multiplication
        if batch_idx % 5 == 0 or batch_idx == len(dataloader) - 1:
            current_acc = 100. * correct / total
            current_loss = running_loss / (batch_idx + 1)
            lr = optimizer.param_groups[0]["lr"]
            
            # Use set_postfix for cleaner updates
    ...
    # Clear the progress bar and print final epoch summary
    progress_bar.close()
...
def validate_epoch(model, dataloader, criterion, device):
    """Validate the model with clean single-line progress bar"""
    model.eval()
    val_loss = 0
    correct = 0
    total = 0
    
    # Create progress bar for validation - simplified for terminal compatibility
    progress_bar = tqdm(
        dataloader, 
        desc='🔍 Validation',
        leave=False,
    ...
    # Clear the progress bar and print final validation summary
    progress_bar.close()
...
def run_deep_training(
    ...
    # Create overall training progress bar - simplified for terminal compatibility
    epoch_progress = tqdm(
        range(1, epochs + 1), 
    ...
    for epoch in epoch_progress:
        ...
        # Update epoch progress bar with metrics (include both loss and accuracy)
        epoch_progress.set_postfix({
            'T_Loss': f'{train_loss:.3f}',
            'T_Acc': f'{train_acc:.1f}%',
            'V_Loss': f'{val_loss:.3f}',
            'V_Acc': f'{val_acc:.1f}%',
            'Best': f'{max(val_accs):.1f}%',
            'LR': f'{current_lr:.1e}'
        })
